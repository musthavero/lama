<div x-data="{addStatusPopUp:false}" class="w-full space-y-4" id="listWithHandle" wire:key="listWithHandle">
    <x-backend::modal>
        <x-slot name="controlVal">addStatusPopUp</x-slot>
        <x-slot name="hideTrigger">close-status-modal</x-slot>
        <x-slot name="showTrigger">show-status-modal</x-slot>
        <x-slot name="title">
            Add Status
        </x-slot>
        <x-slot name="content">
            <livewire:products::add-product-status-form/>
        </x-slot>
    </x-backend::modal>
    ...
    @if ($statuses->count())
        <div class="mx-2 gap-y-2 gap-x-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" id="master-list">
            @each('products::components.one-product-status',$statuses,'item')
        </div>
    @endif
</div>
